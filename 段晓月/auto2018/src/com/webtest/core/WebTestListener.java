...
public class WebTestListener extends TestListenerAdapter{
	...
	public void onFinish(ITestContext testContext) {
		...
		//enable_mail £¿£¿£¿
		try {
			if(BaseTest.getPropertyValue("enable_email").equals("true"))
			{
				String emailContent=this.writeResultToMailTemplate();
				String emailTitle=BaseTest.getPropertyValue("mail_title")+"----"+this.getTime();
				String toMail=BaseTest.getPropertyValue("to_mail");
				try {
					...
					{
						MailUtil.sendEmail(BaseTest.getPropertyValue("success_to_mail"),emailTitle, emailContent);
						Log.info("email send to "+BaseTest.getPropertyValue("success_to_mail")+" success");
					}
